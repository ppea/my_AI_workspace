...
| Dimension | Decision |
|---|---|
| **Primary Interaction** | OpenCode + NextMe MCP Server (zero extra LLM cost, uses existing Copilot/Codex/Claude subscription) |
| **Secondary Interaction** | Telegram for mobile notifications & quick Q&A |
| **Admin Interface** | CLI (`nextme` commands) for reindex, status, memory management |
...
│   └── custom/
│       └── nextme.md             # NextMe Chief of Staff skill
│
├── tests/
│   ├── unit/
│   ├── integration/
...
5. Full reindex: `nextme reindex --full`
...
- Memory deletion is permanent and immediate: `nextme memory forget "..."`
...
| **CLI (`nextme`)** | Admin: reindex, status, memory | **$0** (no LLM) | No |
...
│                 │ calls tools                             │
│  ┌──────────────▼──────────────────────┐                 │
│  │  NextMe MCP Server (NO LLM calls)  │                 │
│  │                                      │                 │
...
**NextMe OpenCode Skill** — contains Chief of Staff routing logic:

```markdown
# skills/custom/nextme.md

You are the Chief of Staff for NextMe.
...
**NextMe MCP Server** — provides tools only, no LLM calls:
...
```bash
nextme reindex [--full]          # rebuild RAG index
nextme status [--costs]          # system health check
nextme memory list [--advisor X] # list memories
nextme memory forget "..."       # delete memory
nextme note add "..." --area X   # add note
nextme remind "..." --date X     # create reminder
```
...
- Advisor-to-project mapping: `NextMe: Health`, `NextMe: Finance`, etc.
...
- No sync needed — Obsidian is the GUI editor, NextMe is the AI brain
...
```
data/logs/
├── nextme.log    # structured JSON log (structlog)
...
Per-advisor, per-day cost tracking. Exposed via `nextme status --costs`.
...
| 1.9 | CLI (admin) | `nextme reindex`, `nextme status`, `nextme note` |
...
| 1.13 | MCP Server | NextMe tools for OpenCode (rag_search, memory, etc.) |
...
**Goal**: NextMe reaches out proactively.
...
💻 Deep work  OpenCode + NextMe MCP ($0 — uses your subscription)
🚶 On-the-go  Telegram bot (quick notes, questions)
🌙 Evening    OpenCode for journaling/review ($0)
🔧 Admin      CLI: nextme reindex / status / memory ($0, no LLM)
```
